...
void ButterflyGenerator::generate(QString part){
    if(part == "ButterflyBoby"){
        ...
        createLeafRules("cyl", part, QVector3D(body_rad, body_len, 20), QVector3D(0,0,0), QVector3D(PI/2,PI/2,0));
    } else if(part == "LeftWing"){
        createLeafRules("cub", part, QVector3D(body_rad,w,body_len/2), QVector3D(-(w+body_rad-0.1),0,0), QVector3D(PI/2,PI/2,0));
